...
ply_data = PlyData.read("pointmap.PLY")
tiff_dir = "IRdata"
...
vertex_stack = np.stack([
    ...
    np.zeros(ply_data['vertex'].data['x'].size)
], axis=-1)
...
def apply_pixel_colors_to_vertices_vectorized(
    ...
    colors = Verticies.T[3, :].copy()
    #print(colors.shape)
    pixel_colors = image.reshape(-1)[selected_pixels]
...
def save_ply_grayscale_as_rgb(vertices, filename):
    ...
    gray = vertices[:, 3].astype(np.uint8)
    data = np.empty(vertices.shape[0], dtype=vertex_dtype)
    data['x'], data['y'], data['z'] = vertices[:, 0], vertices[:, 1], vertices[:, 2]
    data['red'] = data['green'] = data['blue'] = gray

    el = PlyElement.describe(data, 'vertex')
    PlyData([el], text=True).write(filename)
    print(f"Saved {vertices.shape[0]} vertices with grayscale (RGB) color to {filename}")
...
for filename in os.listdir(tiff_dir):
    if filename.lower().endswith(".tiff"):
        ...
        with tifffile.TiffFile(filepath) as tif:
            # Iterate through each page (frame) in the TIFF
            for i, page in enumerate(tif.pages):
                time = tiff_unix_time(page)
                pagepose = posefromtime(pose,time)
                #extract position
                #index 0 is known to be time
                pageposition = np.array([pagepose[1],pagepose[2],pagepose[3]])
                #print(time)
                # There is definitely a faster way to do this utilizing GPU
                pixels = page.asarray()
                # Convert uint16 to 8 bit
                pixels_8bit = ((pixels / 65535) * 255).astype(np.uint8)
                height, width = pixels_8bit.shape
                # qw qx qy qz format
                q_lidar = np.array([pagepose[4],pagepose[5],pagepose[6],pagepose[7]])
                #q_lidar_norm = (q_lidar - q_lidar.min()) / (q_lidar.max() - q_lidar.min())
                ...
                #print(height)
                #print(width)
                vertex_stack = apply_pixel_colors_to_vertices_vectorized(pixels_8bit, height, width, q_image, pageposition, FOVhoz, FOVver, vertex_stack, 1000)
